...
//           serviceFilter === "all" || (lead.service ?? "other") === serviceFilter

//         return matchesSearch && matchesStatus && matchesPriority && matchesService
//       }),
//     [leads, normalizedSearch, searchTerm, statusFilter, priorityFilter, serviceFilter],
//   )
...
//testing new changes (16-12-2025)
...
//   // Step 1: apply filters (search / status / priority / service)
//   const filteredLeads = useMemo(
//     () =>
//       leads.filter((lead) => {
//         const matchesSearch =
...
//   // Step 2: hide converted closed‑won leads from visible list,
//   // but keep them in `leads` so stats still see them.
//   const visibleLeads = useMemo(
//     () =>
//       filteredLeads.filter(
//         (lead) => !(lead.status === "closed-won" && (lead as any).isConverted),
//       ),
//     [filteredLeads],
//   )
...
//   // Stats should use ALL leads, including converted closed‑won, so count on `leads`
//   const stats = useMemo(() => {
//     const total = leads.length
//     const byStatus = {
//       new: leads.filter((l) => l.status === "new").length,
...
//testing new changes (18-12-2025)
// "use client"
...
//   // local UI state, initialised from context filters
//   const [searchTerm, setSearchTerm] = useState("")
...
//   const [userFilter, setUserFilter] = useState<string>(leadFilters.assignedTo ?? "all")
//   const [viewMode, setViewMode] = useState<LeadViewMode>("table")
//   const [selectedLead, setSelectedLead] = useState<Lead | null>(null)
//   const [isAddDialogOpen, setIsAddDialogOpen] = useState(false)
//   const [isEditDialogOpen, setIsEditDialogOpen] = useState(false)
...
//   // keep context filters in sync and refetch ONLY when they actually change
//   useEffect(() => {
//     const next = {
//       status: statusFilter,
...
//       next.assignedTo === leadFilters.assignedTo
//     ) {
//       return
//     }
...
//   // client-side filters including user filter
//   const filteredLeads = useMemo(
//     () =>
//       leads.filter((lead) => {
//         const matchesSearch =
...
//         const matchesUser =
//           userFilter === "all" ||
//           (!lead.assignedTo && userFilter === "all") ||
//           String(lead.assignedTo) === userFilter
...
//   // hide converted closed‑won leads from list, keep for stats
//   const visibleLeads = useMemo(
//     () =>
//       filteredLeads.filter(
//         (lead) => !(lead.status === "closed-won" && (lead as any).isConverted),
...
//   // stats use ALL leads (not filtered)
//   const stats = useMemo(() => {
//     const total = leads.length
//     const byStatus = {
//       new: leads.filter((l) => l.status === "new").length,
...
//             {/* Admin-only user filter, alongside other filters */}
...
//   // Always treat users as an array to avoid optional issues
//   const safeUsers = Array.isArray(users) ? users : []

//   // Local UI state, initialized from context filters
//   const [searchTerm, setSearchTerm] = useState("")
...
//       next.service === leadFilters.service &&
//       next.assignedTo === leadFilters.assignedTo &&
//       next.createdBy === leadFilters.createdBy
//     ) {
//       return
//     }
...
//   // Client-side filtering including createdBy filter
//   const filteredLeads = useMemo(
//     () =>
//       leads.filter((lead) => {
//         const matchesSearch =
...
//         const matchesService =
//           serviceFilter === "all" ||
//           (lead.service ?? "other") === serviceFilter

//         const matchesCreatedBy =
//           createdByFilter === "all" ||
//           String(lead.createdBy) === createdByFilter
...
//           </div>
//           <div className="text-xs text-muted-foreground">
//             Created by: {getUserName(lead.createdBy)}
//           </div>
//           <div className="flex justify-end">
//             <DropdownMenu>
//               <DropdownMenuTrigger asChild>
...
//   // Stats use ALL leads (now includes closed-won because backend no longer hides them)
//   const stats = useMemo(() => {
//     const total = leads.length;
//     const byStatus = {
//       new: leads.filter((l) => l.status === "new").length,
...
export function LeadsContent() {
  ...
  // Helper: get creator name for a lead
  ...
  const filteredLeads = useMemo(
    () =>
      leads.filter((lead) => {
        ...
        const matchesCreatedBy =
          createdByFilter === "all" ||
          String((lead as any).createdBy ?? (lead as any).created_by) ===
            createdByFilter;
